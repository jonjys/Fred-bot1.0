...
lock = json.loads((ROOT / "user_data/PROD_LOCK_V1.json").read_text())
...
if params["stoploss"].get("stoploss") != lock["params"]["stoploss"]:
    errors.append("stoploss differs from PROD LOCK v1")
if prod_strategy.get("prod_lock") != lock["lock_id"]:
...
if live_requested:
    if os.getenv("LIVE_TRADING_ACK") != lock["lock_id"]:
    ...
    has_telegram = bool(os.getenv("FREQTRADE__TELEGRAM__TOKEN") and os.getenv("FREQTRADE__TELEGRAM__CHAT_ID"))
...
if errors:
    ...
    raise SystemExit(1)
print(f"Production preflight OK ({lock['lock_id']}, live_requested={live_requested})")
